...
class Ui_MainWindow(object):
    ...
    def set_progressbar_value(self, value):
        ...
        if value == 100:
            ...
            if self.audioflag:
                self.MergerThread = MergerThread(self.DownloadPath.toPlainText()+'/'+result['title'])
                self.MergerThread.Merger_proess_signal.connect(self.MergerThread_req)
    ...
    def MergerThread_req(self,value):
        if value ==100:
            self.Downloadresult.setText("整合完成")
            os.remove(self.DownloadPath.toPlainText()+'/'+result['title']+'.mp4')
            os.remove(self.DownloadPath.toPlainText()+'/'+result['title']+'.m4a')
            return

class MergerThread(QThread):
    Merger_proess_signal = Signal(int)                        #创建信号

    def __init__(self, VideoFileClip, AudioFileClip, Path):
        super(MergerThread, self).__init__()
        self.Path = Path


    def run(self):
        try:
            clip1 = VideoFileClip(self.Path+'.mp4')
            clip2 = AudioFileClip(self.Path+'.m4a')
            final_clip = clip1.set_audio(clip2)
            final_clip.write_videofile(self.Path+'_final.mp4')
            self.download_proess_signal.emit(100)                #发送信号
            self.exit(0)            #关闭线程
...
class AnalysisThread(QThread):
    ...
    def run(self):
        try:
            ...
            global result
            url=self.url
            # video_page = common.get_content('https://www.youtube.com/watch?v=%s' % query['videoId'])
            ...
            self.analysis_proess_signal.emit(100)
